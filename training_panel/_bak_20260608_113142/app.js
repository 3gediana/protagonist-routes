...
async function refreshMetricFiles() {
  ...
  try {
    ...
    (jsonl.length ? jsonl : list).forEach((f) => {
      const o = document.createElement("option");
      o.value = f.name; o.textContent = `${f.name} (${f.size_kb}KB)`;
      sel.appendChild(o);
    });
...
$("#mAuto").addEventListener("change", (e) => {
  if (e.target.checked) autoTimer = setInterval(loadMetrics, 5000);
  else clearInterval(autoTimer);
});

async function loadMetrics() {
  const proj = $("#mProj").value, file = $("#mFile").value;
  if (!file) return toast("请先选择 jsonl 文件", true);
  try {
...
const gridColor = "rgba(140,154,171,.15)";
const baseScale = { x: { grid: { color: gridColor } }, y: { grid: { color: gridColor } } };
function drawReward(m) {
  const labels = m.reward.map((_, i) => i);
  mk("chReward", { type: "line", data: { labels, datasets: [
    { label: "reward", data: m.reward, borderColor: "rgba(79,157,255,.35)", pointRadius: 0, borderWidth: 1 },
    { label: "reward (smooth)", data: m.reward_smooth, borderColor: "#2bd1c4", pointRadius: 0, borderWidth: 2 },
    { label: "wander ref", data: labels.map(() => m.wander_ref), borderColor: "#ff5d5d", borderDash: [5,5], pointRadius: 0, borderWidth: 1 },
  ]}, options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: "Reward 曲线", color: "#e6edf3" }, legend: { labels: { color: "#8b9aab" } } }, scales: baseScale } });
}
function drawMilestone(m) {
  mk("chMilestone", { type: "bar", data: { labels: m.milestone_names, datasets: [
    { label: "触发率", data: m.milestone_rate.map((x) => +(x*100).toFixed(1)), backgroundColor: "#4f9dff" },
  ]}, options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: "里程碑触发率 %", color: "#e6edf3" }, legend: { display: false } }, scales: baseScale } });
}
function drawAction(m) {
  const colors = m.action_names.map((n) => m.dead_actions.includes(n) ? "#ff5d5d" : "#45d17a");
  ...
  ]}, options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: "动作分布 (红=沉默)", color: "#e6edf3" }, legend: { display: false } }, scales: baseScale } });
}
function drawDeath(m) {
  ...
  mk("chDeath", { type: "doughnut", data: { labels, datasets: [
    { data, backgroundColor: ["#4f9dff","#ffb454","#ff5d5d","#45d17a"] },
  ]}, options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: "死因分布", color: "#e6edf3" }, legend: { labels: { color: "#8b9aab" } } } } });
}
