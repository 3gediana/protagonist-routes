...
(function () {
  ...
  let tm3dBusy = false, tm3dVeilTimer = null;
  ...
  async function loadTmPanorama() {
    ...
    try {
      ...
      else file = (meta.metrics) || (baseName(p.config).replace(/\.[^.]+$/, "") + ".jsonl");
  ...
  async function tmLoad3d() {
    ...
    try {
      if (p.project === "pe") {
        ...
        const dp = (t.dp || []);
        if (!dp.length) { info.textContent = "暂无 DP dump（训练中需 dump 才能渲染；用「新世界」生成）"; tm3dEnd(); return; }
